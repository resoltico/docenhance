...
inline constexpr auto option_catalog = std::to_array<OptionDescriptor>({
    ...
    {.name = "--illumination", .metavar = "MODE", .scope = CommandSet{Command::process}, .group = "Illumination arguments", .domain = "off; off|surface|auto", .methods = "I01", .description = "Select I01 for continuous preserve/gray output only. Off is the default; auto may skip for explicitly reported applicability predicates."},
    {.name = "--background-strength", .metavar = "A", .scope = CommandSet{Command::process}, .group = "Illumination arguments", .domain = "0.35; finite [0,1]", .methods = "I01", .description = "Exponent controlling I01 gain. Zero is an exact photometric no-op after parameter and mask validation."},
    {.name = "--background-max-gain", .metavar = "G", .scope = CommandSet{Command::process}, .group = "Illumination arguments", .domain = "1.5; finite [1,4]", .methods = "I01", .description = "Maximum I01 multiplicative gain. One is an exact photometric no-op."},
    {.name = "--background-target", .metavar = "TARGET", .scope = CommandSet{Command::process}, .group = "Illumination arguments", .domain = "source; source or finite [0.1,1]", .methods = "I01", .description = "Linear-light target. Source uses the fitted background nearest-rank 90th percentile, not forced paper white."},
    ...
    {.name = "--background-smooth", .metavar = "BETA", .scope = CommandSet{Command::process}, .group = "Illumination arguments", .domain = "2; finite [0.1,20]", .methods = "I01", .description = "Positive grid-Laplacian weight for fitting the logarithmic background."},
