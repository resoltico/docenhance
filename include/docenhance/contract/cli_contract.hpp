...
inline constexpr auto option_catalog = std::to_array<OptionDescriptor>({
    {.name = "--out-dir", .metavar = "DIRECTORY", .scope = CommandSet{Command::process}, .group = "Processing arguments", .domain = "Required; a new result directory", .methods = "B03", .description = "Parent must exist. The final directory must not exist; processing writes a staged 8-bit grayscale PNG and publishes the directory only after success."},
    ...
    {.name = "--json", .metavar = "", .scope = CommandSet::all(), .group = "Presentation arguments", .domain = "False", .methods = "", .description = "Writes exactly one final JSON response to stdout."},
