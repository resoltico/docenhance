...
inline constexpr auto option_catalog = std::to_array<OptionDescriptor>({
    ...
    {.name = "--background-strength", .metavar = "A", .scope = CommandSet{Command::process}, .group = "Illumination arguments", .domain = "1; finite [0,1]", .methods = "I01", .description = "Exponent applied to the I01 gain; 1 corrects the fitted background fully within the gain cap. Zero is an exact photometric no-op after parameter and mask validation."},
    {.name = "--background-max-gain", .metavar = "G", .scope = CommandSet{Command::process}, .group = "Illumination arguments", .domain = "2; finite [1,4]", .methods = "I01", .description = "Maximum I01 multiplicative gain. One is an exact photometric no-op."},
