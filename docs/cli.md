...
and enhancement filters remain unsupported.
...
success instead contains `operation: continuous` and a typed `conversion` record, with no fictional
method ID. Both identify the final output path and `publication: completed`. The delivered machine contract is
